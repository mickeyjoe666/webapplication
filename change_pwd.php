...
<?php
				...
				$conn = pg_connect("host=localhost user=j543210a dbname=j543210a");
				$query1 = "SELECT * FROM member WHERE login_name = $1";
				$result = pg_prepare($conn, "change_passwd", $query1);
				$result = pg_execute($conn, "change_passwd", array($login_name));
				$row = pg_fetch_assoc($result, 0);
				if (password_verify($passwd, $row['passwd'])) { // パスワードが正しい
					$newpasswd_hash = password_hash($newpasswd, PASSWORD_DEFAULT);
					$query2 = "UPDATE member SET passwd = $1 WHERE login_name = $2";
					$result1 = pg_prepare ($conn, "update",$query2);
					$result1 = pg_execute ($conn, "update", array($newpasswd_hash,$login_name));
					print "更新しました";
				} else { // パスワードが誤っている
					print "現在のパスワードが間違ってます";
				}
?>
</div> 
<!-- フッター -->
